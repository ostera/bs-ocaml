  ...
  if Ctype.moregeneral env true vd1.val_type vd2.val_type then begin
    match (vd1.val_kind, vd2.val_kind) with
        (Val_prim p1, Val_prim p2) ->
          if p1.prim_name = p2.prim_name &&
            p1.prim_arity = p2.prim_arity &&
            p1.prim_alloc = p2.prim_alloc &&
            p1.prim_native_name = p2.prim_native_name &&
            p1.prim_native_float = p2.prim_native_float 
            (* Can not compare attributes since documenation *)
          then Tcoerce_none else raise Dont_match
